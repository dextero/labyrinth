...
//				at net.sf.fmj.ui.application.ContainerPlayer.createNewPlayer(ContainerPlayer.java:357)
			...
			throws java.io.IOException, NoProcessorException
	{
		...
		for (int i = 0; i < handlerClassList.size(); ++i)
		{
			...
			{
				logger.finer("createProcessor(" + source + ", " + contentType + "): "  + e);	// no need for call stack
				continue;
			}
			catch (NoProcessorException e)
			{	continue;	// no need to log, will be logged by call to createProcessor.
			...
			CannotRealizeException
	{
		...
		processor.setContentDescriptor(model.getContentDescriptor());	// TODO: return value?
			...
			String protocol) throws NoDataSinkException
	{
		...
		for (int i = 0; i < handlerClassList.size(); ++i)
		{
			...
			{
				logger.finer("createDataSink(" + datasource + ", " + protocol + "): "  + e);	// no need for call stack
				continue;
			}
			catch (NoClassDefFoundError e)
			{
...
//		            i.e. look for content-prefix.media.datasink.protocol.content-type.Handler
					...
					for (int j = 0; j < handlerClassList2.size(); ++j)
					{
						...
						{
							logger.finer("createDataSink(" + datasource + ", " + destLocator + "), proxy=" + mediaProxy.getDataSource() + ": "  + e);	// no need for call stack
							continue;
						}
						catch (NoClassDefFoundError e)
						{
			...
			{
				logger.finer("createDataSink(" + datasource + ", " + destLocator + "): "  + e);	// no need for call stack
				continue;
			}
			catch (NoClassDefFoundError e)
			{
